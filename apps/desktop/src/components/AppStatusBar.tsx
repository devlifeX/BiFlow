...
import { ConnectionProgressStrip } from "./ConnectionProgressStrip";
...
export function AppStatusBar() {
  ...
  const installingId = useAppStore((state) => state.installingId);
  ...
  return (
    <footer
      className="app-status-bar sticky bottom-0 z-20 shrink-0 border-t border-[rgb(var(--border-default))] bg-[rgb(var(--toolbar))] text-[11px] text-muted"
      role="status"
      aria-live="polite"
      title={current.detail ?? undefined}
    >
      <ConnectionProgressStrip
        snapshot={snapshot}
