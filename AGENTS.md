...
- Switching the live default route from DIRECT to a local proxy can select a
  proxy that was optional and had no handle during initial Connect. A live
  settings apply must run local-proxy recovery before staging Mihomo, and
  recovery must update the primary egress IP when that client is now the
  default; otherwise readiness reports Hiddify unavailable even while its
  proxy answers (ADR 0076).
...
- Native Windows packaging uses `build.ps1`; keep it limited to the portable `.exe` and NSIS installer and aligned with `build.sh` for version, stages, helper staging, and artifact names (ADR 0087).
- Windows script tests must avoid Unix shell spawns on `win32`, accept CRLF checkout lines in YAML/Rust/lockfile assertions, and invoke Python through `py -3` instead of the Microsoft Store `python3` alias. Otherwise the Windows test job reports false failures before testing project behavior.
- Windows helper reinstalls write `C:\ProgramData\iran-split\helper.toml` under elevation; retry sharing/access errors briefly because Defender or the previous helper can hold the machine-wide file during reinstall, then preserve the original I/O cause (ADR 0088).
...
- A live default-route change hot-reloads Mihomo but leaves existing sockets on the previous outbound, so the diagram keeps drawing them on Hiddify. Close every connection when the reload has no single-host rebind, and drop a diagram packet as soon as a newer poll places that host on another branch (ADR 0100).
